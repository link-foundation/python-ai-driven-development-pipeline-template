...
# GitHub's concurrency handling, which is expected churn (issue #69). When the
# head cannot be resolved at all (missing env, unreachable remote), the run is
# treated as NOT superseded and the check fails loud -- a silent pass here
# would be the exact bug this gate exists to prevent.
set -euo pipefail
